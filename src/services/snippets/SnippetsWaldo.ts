...
export default class SnippetsWaldo implements ISnippets {
  ...
  getSnippetFolder(objectType: ObjectType): string {
    if (
      this._unsupportedObjectTypes.findIndex((x) => x === objectType) !== -1
    ) {
  ...
  showALObjectSnippet(
    ...
    if (
      ...
      return SnippetsDefault.instance.showALObjectSnippet(
        objectType,
        firstSnippetPrefix,
    ...
    return currentSnippetPrefix.startsWith(
      firstSnippetPrefix.substring(0, firstSnippetPrefix.indexOf("waldo")),
    );
  }
}
